...
#define BLYNK_TEMPLATE_ID "TMPL4oNzhCcFW"
#define BLYNK_TEMPLATE_NAME "Wheelvy"
#define BLYNK_AUTH_TOKEN "d1rnJY8vYBvJkg6BCbF4UOL4LYUKxR0H"
...
#define V_IR_SENSOR_PIN  V8
#define V_EMERGENCY_PIN  V9
#define V_SECURITY_SWITCH V10

#define GAS_SENSOR_TIMEOUT 60000
...
BLYNK_WRITE(V_SECURITY_SWITCH) {
  isSecurityArmed = param.asInt();
}
...
void all_SwitchOff(){
  ...
  Blynk.virtualWrite(V_IR_SENSOR_PIN, (!irSensorState ? "Car on base" : "Car is missing"));
  Blynk.virtualWrite(V_EMERGENCY_PIN, (isEmergency ? "Warning!" : "All good"));
}
...
void checkBlynkStatus() { // called every 3 seconds by SimpleTimer
  ...
  if (isconnected == true) {
    ...
    Blynk.virtualWrite(V_SECURITY_SWITCH, isSecurityArmed);
    Blynk.virtualWrite(V_IR_SENSOR_PIN, (!irSensorState ? "Car on base" : "Car is missing"));
    Blynk.virtualWrite(V_EMERGENCY_PIN, (isEmergency ? "Warning!" : "All good"));
    }
...
BLYNK_CONNECTED() {
  ...
  if (fetch_blynk_state){
    ...
    Blynk.syncVirtual(V_IR_SENSOR_PIN);
    Blynk.syncVirtual(V_EMERGENCY_PIN);
    Blynk.syncVirtual(V_SECURITY_SWITCH);
}
...
void IRAM_ATTR G_S_Sensor_ISR() {
    if (!digitalRead(G_S_SENSOR_PIN) && isSecurityArmed && millis()-gasTimer > GAS_SENSOR_TIMEOUT) isEmergency = true;
}
void IRAM_ATTR WaterSensor_ISR() {
    if(analogRead(IR_SENSOR_PIN) > 150 && isSecurityArmed) isEmergency = true;

}
void IrSensorCheck() {    
    if (sensorStateChanged) 
    {
        if (irSensorState != digitalRead(IR_SENSOR_PIN))
        {
          irSensorState = digitalRead(IR_SENSOR_PIN);
        }
...
void setup()
{
  ...
  pinMode(G_S_SENSOR_PIN, INPUT);
  pinMode(WATER_SENSOR_PIN, INPUT);

  attachInterrupt(IR_SENSOR_PIN, IrSensor_ISR, CHANGE);
  attachInterrupt(G_S_SENSOR_PIN, G_S_Sensor_ISR, FALLING);
  attachInterrupt(WATER_SENSOR_PIN, WaterSensor_ISR, RISING); 
